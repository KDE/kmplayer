...
KDE_NO_CDTOR_EXPORT ControlPanel::ControlPanel(QWidget * parent, View * view)
 : QWidget (parent),
   ...
   m_popup_clicked (false) {
    ...
    m_buttons[button_language] = ctrlButton (this, m_buttonbox, language_xpm);
    ...
    connect (m_buttons [button_config], SIGNAL (clicked ()),
            this, SLOT (ctrlButtonClicked ()));
    connect (m_buttons [button_config], SIGNAL (mouseEntered ()),
             this, SLOT (ctrlButtonMouseEntered ()));
    connect (m_popupMenu, SIGNAL (mouseLeft ()),
             this, SLOT (popupMenuMouseLeft ()));
    connect (m_playerMenu, SIGNAL (mouseLeft ()),
             this, SLOT (popupMenuMouseLeft ()));
    connect (m_zoomMenu, SIGNAL (mouseLeft ()),
             this, SLOT (popupMenuMouseLeft ()));
    connect (m_colorMenu, SIGNAL (mouseLeft ()),
             this, SLOT (popupMenuMouseLeft ()));
}
...
KDE_NO_EXPORT void ControlPanel::timerEvent (QTimerEvent * e) {
    if (e->timerId () == m_popup_timer) {
        m_popup_timer = 0;
        if (m_buttons [button_config]->hasMouse() && !m_popupMenu->isVisible ())
            showPopupMenu ();
    } else if (e->timerId () == m_popdown_timer) {
        m_popdown_timer = 0;
        if (m_popupMenu->isVisible () && !m_popupMenu->hasMouse () && !m_playerMenu->hasMouse () && !m_zoomMenu->hasMouse () && !m_colorMenu->hasMouse () && !m_bookmarkMenu->hasMouse ())
            if (!(m_bookmarkMenu->isVisible () && static_cast <QWidget *> (m_bookmarkMenu) != QWidget::keyboardGrabber ()))
...
KDE_NO_EXPORT void ControlPanel::ctrlButtonClicked () {
    if (m_popup_timer) {
...
KDE_NO_EXPORT void ControlPanel::ctrlButtonMouseEntered () {
    if (!m_popup_timer && !m_popupMenu->isVisible ()) {
        m_popup_clicked = false;
        m_popup_timer = startTimer (400);
    }
}

KDE_NO_EXPORT void ControlPanel::popupMenuMouseLeft () {
    if (!m_popdown_timer && !m_popup_clicked)
        m_popdown_timer = startTimer (400);
}

KDE_NO_EXPORT void ControlPanel::setSubtitles (const QStringList &) {
}

KDE_NO_EXPORT void ControlPanel::setAudioLanguages (const QStringList &) {
}

KDE_NO_EXPORT void ControlPanel::selectSubtitle (int id) {
}

KDE_NO_EXPORT void ControlPanel::selectAudioLanguages (int id) {
}
